...
import "./TreeProsperityPool.sol";
import "./Types.sol";

contract TreeNFT is Ownable, ReentrancyGuard {
  ...
  mapping(uint256 => Types.NFT) public treeNfts;
  ...
  mapping(Types.StakePeriod => TreeProsperityPool) public stakingPools;
  ...
  string public constant name = "Tree NFT";
  ...
  constructor(
    ...
    stakingPools[Types.StakePeriod.OneYear] = new TreeProsperityPool(
      _owner,
      365 days
    );
    stakingPools[Types.StakePeriod.ThreeYears] = new TreeProsperityPool(
      _owner,
      365 days * 3
    );
    stakingPools[Types.StakePeriod.FiveYears] = new TreeProsperityPool(
      _owner,
      365 days * 5
    );
  ...
  function renewTreeNFT(
    ...
    _renewTreeNFT(_id, _months);
  }

  function renewTreeNFTBatch(
  ...
  function _renewTreeNFT(uint256 _id, uint256 _months) internal {
    Types.NFT storage nft = treeNfts[_id];
  ...
  function getPendingCREDIT(uint256 _id) public view returns (uint256) {
    Types.NFT memory nft = treeNfts[_id];
  ...
  function removeNft(uint256 _id) external onlyOwner {
    uint256 lastNFTIndex = balanceOf[treeNfts[_id].minter];
    uint256 nftIndex = ownedNftsIndex[_id];

    if (nftIndex != lastNFTIndex) {
      uint256 lastNFTId = ownedNfts[treeNfts[_id].minter][lastNFTIndex];

      ownedNfts[treeNfts[_id].minter][nftIndex] = lastNFTId; // Move the last NFTto the slot of the to-delete token
      ownedNftsIndex[lastNFTId] = nftIndex; // Update the moved NFT's index
    }

    // This also deletes the contents at the last position of the array
    delete ownedNftsIndex[_id];
    delete ownedNfts[treeNfts[_id].minter][lastNFTIndex];

    balanceOf[treeNfts[_id].minter]--;
    totalNFTs--;
